...
;;; TODO:
;;; 添加 C-u C-x C-e 将执行结果写入当前 buffer 的功能
;;; 补全
;;; gjs 支持
;;; $NODE_PATH 重复添加问题
...
(defun js-eval-last-sexp ()
  "执行最后一个表达式，如果 region 存在的话则执行 region 中的内容"
  (interactive)
  (if (use-region-p)
      (call-interactively #'js-comint-send-region)
    (call-interactively #'js-send-last-sexp)))

;;;###autoload
(defun js-eval-current-defun ()
  "执行当前 defun"
  (interactive)
  (save-excursion
    (let ((end (progn (js-end-of-defun) (point))))
