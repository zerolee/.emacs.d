...
(defun ove--html-tag (N)
  ...
  (sgml-skip-tag-backward 1)
  (while (member (buffer-substring-no-properties (point) (+ (point) 4))
                 '("<br>" "<!--" "<met" "<lin" "<img" "<are"))
    (sgml-skip-tag-backward 1))
...
(define-key ove-mode-map (kbd "R") #'(lambda () (interactive)
                                       ...
                                       (setq cursor-type 'box)
                                       (overwrite-mode 1)))
(define-key ove-mode-map (kbd "s") #'avy-goto-char-in-line)
(define-key ove-mode-map (kbd "t") #'avy-goto-char)
